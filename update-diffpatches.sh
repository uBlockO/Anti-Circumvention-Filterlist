...
for PATCH_FILE in "${PATCH_FILES[@]}"; do
    ...
    OLD_REPO=$(mktemp -d)
    git clone -q --single-branch --branch $PREVIOUS_VERSION --depth=1 https://github.com/uBlockO/Anti-Circumvention-Filterlist.git "$OLD_REPO" 2>/dev/null

    > "$NEW_PATCH_FILE"

    FILTER_LISTS=( $(ls -1 acf.txt) )
